...
export function ProductFilter({ initialProducts }: { initialProducts: Product[] }) {
  ...
  const uniqueLokasi = Array.from(
    new Set(initialProducts.map((p) => p.lokasi).filter((lokasi): lokasi is string => lokasi !== null))
  ).sort();

  const uniqueKategori = Array.from(
    new Set(initialProducts.map((p) => p.kategori).filter((kategori): kategori is string => kategori !== null))
  ).sort();
